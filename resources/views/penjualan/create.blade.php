    ...
    <div class="card shadow-sm p-3">
        <form action="{{ route('penjualan.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="date" name="tanggal_penjualan" id="tanggal" class="form-control" value="{{ old('tanggal_penjualan', date('Y-m-d')) }}" required>
            </div>
            ...
            <table class="table" id="produk-table">
                <thead>
                    <tr>
                        ...
                        <th>Subtotal</th>
                        <th>
                            <button type="button" class="btn btn-sm btn-success" id="add-row">Tambah</button>
                        </th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        ...
                        <td><input type="number" name="jumlah_produk[]" class="form-control jumlah-input" min="1" value="1" required></td>
                        <td><input type="text" class="form-control harga-satuan" readonly></td>
            ...
            <div class="mb-3 d-flex gap-3 align-items-end">
                <div class="flex-fill">
                    ...
                    <input type="text" name="total_harga" id="total_harga" class="form-control" readonly value="{{ old('total_harga', '0') }}">
                </div>
                <div class="flex-fill">
                    <label for="bayar" class="form-label">Bayar</label>
                    <input type="number" name="bayar" id="bayar" class="form-control" min="0" value="{{ old('bayar', '0') }}" required>
                </div>
                <div class="flex-fill">
                    <label for="kembalian" class="form-label">Kembalian</label>
                    <input type="text" name="kembalian" id="kembalian" class="form-control" readonly value="{{ old('kembalian', '0') }}">
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    function updateRow(row) {
        ...
        const harga = produkSelect.selectedOptions[0]?.dataset.harga ? parseFloat(produkSelect.selectedOptions[0].dataset.harga) : 0;
        const jumlah = parseInt(jumlahInput.value) || 0;
        const subtotal = harga * jumlah;

        hargaSatuanInput.value = harga ? harga.toLocaleString('id-ID', { style: 'currency', currency: 'IDR' }) : '';
        subtotalInput.value = subtotal ? subtotal.toLocaleString('id-ID', { style: 'currency', currency: 'IDR' }) : '';
    ...
    function updateTotalHarga() {
        ...
        document.getElementById('total_harga').value = total.toLocaleString('id-ID', { style: 'currency', currency: 'IDR' });
        updateKembalian();
    }
    ...
    function updateKembalian() {
        ...
        kembalianInput.value = kembalian >= 0 ? kembalian.toLocaleString('id-ID', { style: 'currency', currency: 'IDR' }) : '0';
    }
